...
For local non-Docker usage, see `README_LOCAL.md`.
...
This repository now also works outside Docker with a project-local conda environment.

```bash
cd diffusion-for-shared-autonomy
conda activate /absolute/path/to/diffusion-for-shared-autonomy/.conda/env
...
The helper script below wraps the validated local commands:
...
`full-smoke` was validated on `LunarLander-v1` with the following stages:
1. `train_sac`
2. `generate_data`
3. `diffusion.train`
4. `eval_assistance`
