...
class ReplayBuffer:
    def __init__(self, directory, state_dim, action_dim, chunk_size=10_000) -> None:
        ...
        for fname in self.directory.iterdir():
            ...
            self._file_cache[fname] = torch.load(
    ...
    def store(
        ...
        entry = np.concatenate((state, action, q_val)).astype(np.float32, copy=False)
    ...
    def dump_buffer(self):
        ...
        chunk = self._current_buff[: self.pointer].copy()
        torch.save(chunk, fname)
        self._file_cache[fname] = chunk
...
def main(save_video):
    ...
    if "LunarLander" in DCArgs.env_name:
        ...
        modeldir = Path(DCArgs.lunarlander_sac_model_dir) / lvl
        datadir = (
            Path(DCArgs.lunarlander_data_dir) / lvl / f"randp_{DCArgs.randp:.1f}"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--env-name", type=str, default=None, help="override env name")
    parser.add_argument("--num-transitions", type=int, default=None, help="override number of transitions")
    parser.add_argument("--valid-return-threshold", type=float, default=None, help="override return threshold")
    parser.add_argument("--randp", type=float, default=None, help="override random action probability")
    parser.add_argument("--user-goal", type=str, default=None, help="override block push user goal")
    parser.add_argument("--seed", type=int, default=None, help="override random seed")
    video_group = parser.add_mutually_exclusive_group()
    video_group.add_argument("--save-video", dest="save_video", action="store_true", help="save rollout video to wandb")
    video_group.add_argument("--no-save-video", dest="save_video", action="store_false", help="disable rollout video logging")
    ...
    if args.env_name is not None:
        DCArgs.env_name = args.env_name
    if args.num_transitions is not None:
        DCArgs.num_transitions = args.num_transitions
    if args.valid_return_threshold is not None:
        DCArgs.valid_return_threshold = args.valid_return_threshold
    if args.randp is not None:
        DCArgs.randp = args.randp
    if args.user_goal is not None:
        DCArgs.blockpush_user_goal = args.user_goal
    if args.seed is not None:
        DCArgs.seed = args.seed
    ...
    if save_video is None:
        save_video = os.environ.get("WANDB_MODE", "").lower() not in {
            "disabled",
            "disable",
        }
    main(save_video=save_video)
    wandb.finish()
