...
def main():
    ...
    num_workers = int(os.environ.get("DIFFUSHA_NUM_WORKERS", "0"))
    loader = iter(DataLoader(dataset, batch_size=Args.batch_size, num_workers=num_workers))
...
if __name__ == "__main__":
    ...
    parser.add_argument("sweep_file", nargs="?", default=None, help="sweep file")
    parser.add_argument("--sweep-file", dest="sweep_file_opt", default=None, help="sweep file")
    parser.add_argument(
        "-l", "--line-number", type=int, help="line number of the sweep-file"
    )
    parser.add_argument("--num-training-steps", type=int, default=None, help="override training steps")
    parser.add_argument("--save-every", type=int, default=None, help="override checkpoint interval")
    parser.add_argument("--eval-every", type=int, default=None, help="override evaluation interval")
    parser.add_argument("--batch-size", type=int, default=None, help="override batch size")
    parser.add_argument("--randp", type=float, default=None, help="override dataset random-action ratio")
    ...
    sweep_file = args.sweep_file_opt or args.sweep_file
    if sweep_file is None:
        parser.error("a sweep file must be provided either positionally or via --sweep-file")

    # Obtain kwargs from Sweep and update hyperparameters accordingly
    sweep = Sweep(Args).load(sweep_file)
    kwargs = list(sweep)[args.line_number]
    Args._update(kwargs)

    if args.num_training_steps is not None:
        Args.num_training_steps = args.num_training_steps
    ...
    if args.randp is not None:
        Args.randp = args.randp

    sweep_basename = Path(sweep_file).stem
