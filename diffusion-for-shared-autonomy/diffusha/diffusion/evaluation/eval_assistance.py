...
def eval_original_actors(make_env: Callable, expert_agent, num_episodes: int = 10,
                         save_video: bool = False, histogram: bool = False,
                         diffusion_actor: bool = False, use_vector_env=False, num_envs=1,
                         laggy_actor_repeat_prob: float = 0.8, noisy_actor_eps: float = 0.8):
    sample_env = make_env()
    obs_space = sample_env.observation_space
    act_space = sample_env.action_space
...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-step", type=int, default=None, help="checkpoint step to evaluate; defaults to the latest checkpoint")
    ...
    if args.out_dir is not None:
        Args.results_dir = str(Path(args.out_dir).expanduser().resolve())
    ...
    diffusion = prepare_diffusha(
        ...
        args.model_step if args.model_step is not None else env2step[args.env_name],
        args.env_name,
        fwd_diff_ratio,
